...
import { getCajaFinal, URL } from "../api";
...
const CajaFinalStatus = ({ route }) => {
  ...
  const loadCaja = async () => {
    if (route && route.params._id) {
      // console.log(route.params._id);
      const data = await getCajaFinal(route.params._id);
      setCajaActual(data);
      let arrayProductos = [];
      let arrayBotellas = [];
      let arrayCopas = [];
      ...
      const foundRefrescosTotal = data.comandas.map((elementComanda) =>
      {
      ...
      const resultado3 = {};
      arrayRefrescos.forEach(
        (el) => (resultado3[el] = resultado3[el] + 1 || 1)
      );
      ...
      const foundReferenciasTotal = data.comandas.map((element) => {
...
//               }}
//             >
//               <Text style={{fontSize: 32, fontWeight: 'bold'}}>Caja Total:</Text>
//               <Text style={{fontSize: 24}}>{cajaActual.dineroCaja} €</Text>
//             </View>
//             <View
//               style={{ width: "100%", height: "85%", paddingHorizontal: 5 }}
